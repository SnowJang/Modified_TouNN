...
class TopOptLoss(nn.Module):
    ...
    def forward(self, nn_rho, Jelem, desiredVolumeFraction, penal, obj0):
        objective = torch.norm(Jelem, p=2) / obj0
...
class TopologyOptimizer:
    ...
    def optimizeDesign(self, maxEpochs, minEpochs, useSavedNet):
        Ploteveryepoch = True
        self.convergenceHistory = [];
        savedNetFileName = "Initial_parameter.nt";
        ...
        for epoch in range(maxEpochs):
            ...
            nn_rho_2 = nn_rho.to(torch.device("cpu")).clone()
            u, Jelem, Q = self.FE.solve88(rho_np, nn_rho, self.E_target);  # Call FE 88 line code [Niels Aage 2013]
            if (epoch == 0):
                self.obj0 = torch.norm(Jelem, p=2)
            ...
            if Ploteveryepoch:
                    ...
                    plt.close()
                    print(
                        "{:3d} J: {:.2F}; Vf: {:.3F}; loss: {:.3F}; relGreyElems: {:.3F} ; E11: {:.4F} ; E12: {:.4F} ; E13: {:.4F} ; E21: {:.4F} ; E22: {:.4F} ; E23: {:.4F} ; E31: {:.4F} ; E32: {:.4F} ; E33: {:.4F}" \
                        .format(epoch, self.objective.item() , currentVolumeFraction, loss.item(),
                                relGreyElements, Q[0,0], Q[0,1], Q[0,2], Q[0,3], Q[0,4], Q[0,5], Q[0,6], Q[0,7], Q[0,8]));
                ...
                if (epoch % 20 == 0):
                    ...
                    self.mid_plotConvergence();
                    plt.close()
                    print("{:3d} Obj: {:.2F}; Vf: {:.3F}; loss: {:.3F}; relGreyElems: {:.3F} ; E11: {:.4F} ; E12: {:.4F} ; E13: {:.4F} ; E21: {:.4F} ; E22: {:.4F} ; E23: {:.4F} ; E31: {:.4F} ; E32: {:.4F} ; E33: {:.4F}" \
                          .format(epoch, self.objective.item(), currentVolumeFraction, loss.item(),
                                  relGreyElements, Q[0], Q[1], Q[2],Q[3],Q[4], Q[5],Q[6],Q[7],Q[8]));
            if ((epoch > minEpochs) & (abs(self.objective - self.afterobjective )/self.objective <1e-4)):  #(relGreyElements < 0.035)): # (self.objective.cpu().detach().numpy().astype(np.float)< 0.03)) : #(relGreyElements < 0.035)):
                break;
        self.plotTO(epoch, True);
        print("{:3d} J: {:.2F}; Vf: {:.3F}; loss: {:.3F}; relGreyElems: {:.3F} " \
              .format(epoch, self.objective.item() , currentVolumeFraction, loss.item(), relGreyElements));
        torch.save(self.topNet, "Initial_parameter_2.nt");
    ...
    def plotTO(self, iter, saveFig=False):
        ...
        c= copy.deepcopy(b)
        u, Jelem, Q = self.FE.solve88(density, torch.tensor(density), self.E_target)
        #Jelem = Jelem = torch.tensor(Jelem).clone().detach().requires_grad(True)
        density_2=torch.tensor(density).view(-1);
        objective, volConstraint = \
            self.lossFunction(density_2, Jelem, self.desiredVolumeFraction, self.FE.penal, self.obj0);
        self.afterobjective = objective
        ...
        plt.title('Iter = {:d}, Obj = {:.2F}, V_f = {:.2F}, V_des = {:.2F}'.format(iter + 1 , objective, #self.objective ,
                                                                                 np.mean(density),
...
maxEpochs = 1000;  # Max number of iterations
penal = 3;  # SIMP penalization constant, starting value
useSavedNet = torch.load('Initial_parameter.nt');  # use a net previouslySaved  as starting point (exampleName_nelx_nely.nt in ./results folder)
#  ~~~~~~~~~~~~ Examples ~~~~~~~~~~~~~#
